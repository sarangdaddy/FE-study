...
> `Object.getOwnPropertyDescriptors` 를 사용하면 모든 프로퍼티 어트리뷰트 정보를 제공한다.
...
- `데이터 프로퍼티(data property)`는 `키`와 `값`으로 구성된 일반적인 프로퍼티다.
...
- `접근자 프로퍼티(accessor property)`는 자체적으로는 값을 갖지 않고 다른 데이터 프로퍼티의 값을 읽거나 저장할 때 사용하는 `접근자 함수(accessor function)`으로 구성된 프로퍼티다.
...
2. `프로토타입 체인`에서 프로토타입을 검색한다. person 객체에 fullName 프로퍼티가 존재한다.
...
- 객체가 변경되면 곤란한 상황이 있는 경우를 대비해 객체 변경을 방지하는 방법이 존재한다.
...
- 프로퍼티는 프로퍼티 `동적 추가`와 `Object.defineProperty` 메서드로 추가할 수 있는데 이 두 가지 추가 방법이 모두 금지된다.
- 확장이 가능한 객체인지 여부는 `Object.isExtensible`로 확인할 수 있다.
...
- 확장이 가능한 객체인지 여부는 `Object.isSealed` \*\*\*\*메서드로 확인할 수 있다.
...
- 즉, **동결된 객체는 읽기만 가능하다.**
- 확장이 가능한 객체인지 여부는 `Object.isFrozen` 메서드로 확인할 수 있다.
...
> ❗️위 메서드들은 얕은 변경 방지shallow only로 직속 프로퍼티만 변경이 방지되고 중첩 객체까지는 영향을 주지 못한다.

### ☝️ 깊은 객체 동결 방법

- 객체의 중첩 객체까지 동결하여 변경이 불가능한 `읽기 전용` 불변 객체
- 객체를 값으로 갖는 모든 프로퍼티에 대해 재귀적으로 Object.freeze 메서드를 호출한다.
